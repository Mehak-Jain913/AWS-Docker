...
export class MonacoBinding {
   ...
   * @param {any} [awareness]
   * @param {any} monacoInstance
   */
  constructor (ytext, monacoModel, editors = new Set(), awareness = null, monacoInstance) {
    if (!monacoInstance) {
      throw new Error('Monaco instance is required for MonacoBinding initialization.')
    }
    ...
    /**
     * @type {Map<any, RelativeSelection>}
     */
    ...
    this._ytextObserver = event => {
      this.mux(() => {
        let index = 0
        event.delta.forEach((/** @type {{ retain: number; insert: string; delete: number; }} */ op) => {
    ...
    this._monacoChangeHandler = monacoModel.onDidChangeContent((/** @type {{ changes: any[]; }} */ event) => {
